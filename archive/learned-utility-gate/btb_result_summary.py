...
def label_for(path: Path, payload: dict) -> str:
    ...
    if source == "jsonl_dataset":
        dataset_dir = args.get("jsonl_dataset_dir")
        return Path(dataset_dir).name if dataset_dir else source
    if source == "burstgpt_csv":
        burstgpt_url = args.get("burstgpt_url")
        return Path(burstgpt_url).name if burstgpt_url else source
    if source and source != "mooncake_parquet":
        return source
