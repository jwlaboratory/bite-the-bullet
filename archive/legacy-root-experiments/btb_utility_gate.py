...
def evaluate_policy_set(
    ...
    rows = {"baseline": [], "candidate_all": [], "oracle_utility": [], "trained_gate": []}
    for window in windows:
        rows["baseline"].append(baselines[window.obs[0].window_id])
        candidate_ids = set(window.candidates)
        oracle_ids = {ex.request_id for ex in by_window_examples[window.obs[0].window_id] if ex.label}
        trained_ids = {
            ex.request_id
            for ex in by_window_examples[window.obs[0].window_id]
...
def main() -> None:
    ...
    print(
        ...
        f"dataset={args.dataset}, config={args.config_name}, objective={args.objective_metric}",
        flush=True,
    )
