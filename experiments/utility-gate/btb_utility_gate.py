...
Node model and local BTB warming mechanics, including fake-prefill warming and
RDMA-prefetch warming, because the main simulator does not expose speculative
warming as a policy.
"""
...
def make_cfg(args: argparse.Namespace) -> SimpleNamespace:
    ...
    if args.mbu > 0:
        cfg.MBU = args.mbu
    cfg.WARM_TRANSPORT = args.warm_transport
...
def load_windows(args: argparse.Namespace, cfg: SimpleNamespace) -> list[Window]:
    if args.source == "jsonl_dataset":
        return load_jsonl_dataset_windows(args, cfg)
...
def build_window(
    ...
    obs = normalize_jsonl_window(rows, window_id, args)
    requests = requests_from_obs(obs, cfg)
    features = build_feature_rows(obs, cfg, args.warm_blocks, args.horizon_s)
    fcounts = future_counts(obs, args.horizon_s)
    raw_candidates = [
    ...
    positives = sum(1 for i in raw_candidates if fcounts.get(i, 0) >= args.future_k)
    print(
        f"window {window_id:02d} {ident:<32} rows={len(rows):<5} "
        f"span={obs[-1].t if obs else 0:.1f}s candidates={len(raw_candidates):<4} "
        f"future>=k={positives:<4}",
        flush=True,
    )
    return Window(ident, obs, requests, features, fcounts, raw_candidates)


def iter_jsonl_trace_groups(path: Path, *, limit: int | None = None):
    current_trace = None
    current_rows = []
    yielded = 0
    with path.open("r", encoding="utf-8") as fh:
        for line in fh:
            ...
            if trace_id != current_trace:
                yield current_trace, current_rows
                yielded += 1
    ...
    if current_trace is not None and (limit is None or yielded < limit):
        yield current_trace, current_rows


def load_jsonl_dataset_windows(args: argparse.Namespace, cfg: SimpleNamespace) -> list[Window]:
    dataset_dir = Path(args.jsonl_dataset_dir)
    train_path = dataset_dir / "train.jsonl"
    test_path = dataset_dir / "test.jsonl"
    if not train_path.exists() or not test_path.exists():
        raise RuntimeError(f"{dataset_dir} must contain train.jsonl and test.jsonl")

    train_limit = args.train_windows
    test_limit = args.windows - args.train_windows
    windows = []
    for trace_id, rows in iter_jsonl_trace_groups(train_path, limit=train_limit):
        windows.append(
            build_window(
                f"train:{trace_id}",
                rows,
    ...
    if len(windows) != train_limit:
        raise RuntimeError(f"expected {train_limit} train windows, found {len(windows)}")
    for trace_id, rows in iter_jsonl_trace_groups(test_path, limit=test_limit):
        windows.append(
            build_window(
                f"test:{trace_id}",
                rows,
    ...
    if len(windows) != args.windows:
        raise RuntimeError(f"expected {args.windows} total windows, found {len(windows)}")
    return windows
...
def schedule_rdma_prefetch(
    ...
    warm_bytes = 0
    warm_busy_s = 0.0
    nbytes = len(blocks) * targets[0].block_bytes if targets else 0
    for target in targets:
...
def run_policy(
    ...
    *,
    ...
    for req in window.requests:
        ...
        if key and active_keys.get(key, -1.0) >= now:
            if cfg.WARM_TRANSPORT == "rdma":
                new_pending, warm_stats = schedule_rdma_prefetch(
                    req,
                    nodes,
                    now,
                    key_blocks,
...
def early_burst_trigger_ids(
    ...
    *,
    ...
    if burst_count <= 1 or burst_window_s <= 0:
        return set()

    histories: dict[tuple[str, ...], deque[float]] = defaultdict(deque)
    active_until: dict[tuple[str, ...], float] = {}
    triggers: list[int] = []
    for item in window.obs:
        if not item.key:
            continue
        q = histories[item.key]
        pop_old(q, item.t, burst_window_s)
        recent_including_current = len(q) + 1
        if recent_including_current >= burst_count and active_until.get(item.key, -1.0) < item.t:
            triggers.append(item.row_in_window)
            active_until[item.key] = item.t + cooldown_s
...
def evaluate_policy_set(
    ...
    for window in windows:
        ...
        early_ids = early_burst_trigger_ids(
            window,
            burst_count=args.early_burst_count,
            burst_window_s=args.early_burst_window_s,
            cooldown_s=args.early_burst_cooldown_s or args.active_ttl_s,
            max_triggers=args.early_burst_max_triggers,
        )
        for name, ids in [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source", choices=["mooncake_parquet", "burstgpt_csv", "jsonl", "jsonl_dataset"], default="mooncake_parquet")
    ...
    parser.add_argument("--jsonl-dataset-dir")
    ...
    parser.add_argument("--rdma-on-admit", action="store_true")
    ...
    parser.add_argument(
        "--warm-transport",
        choices=["prefill", "rdma"],
        default="prefill",
        help="How predictive warming materializes KV: recompute prefill on targets, or RDMA-prefetch existing KV into target HBM.",
    )
    ...
    parser.add_argument(
        "--candidate-sort",
        choices=["history", "future"],
        default="history",
        help="Candidate cap ordering for jsonl_dataset windows; future is for offline balanced eval sampling.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Also evaluate a reactive policy that triggers on the Nth same-prefix arrival inside --early-burst-window-s.",
    )
    parser.add_argument("--early-burst-window-s", type=float, default=0.2)
    parser.add_argument(
        "--early-burst-cooldown-s",
        type=float,
        default=0.0,
        help="Suppress repeat early-burst triggers for this long per key; <=0 uses --active-ttl-s.",
    )
    parser.add_argument(
        "--early-burst-max-triggers",
        type=int,
        default=0,
...
def main() -> None:
    ...
    if args.source == "jsonl_dataset" and not args.jsonl_dataset_dir:
        raise SystemExit("--jsonl-dataset-dir is required when --source jsonl_dataset")
    cfg = make_cfg(args)
    cfg.RDMA_ON_ADMIT = args.rdma_on_admit
    ...
    print(
        ...
        f"warm_transport={args.warm_transport}, rdma_on_admit={args.rdma_on_admit}",
        flush=True,
    )
